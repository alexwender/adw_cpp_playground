...
TEST(DataObjectTest, create_simple_data_object) {
    ...
    ASSERT_FLOAT_EQ(data_obj.getData_1(), 1.0);
    ASSERT_FLOAT_EQ(data_obj.getData_2(), 2.0);

    ASSERT_EQ(data_obj.CONSTRUCTOR_CALLED, 1);

}

TEST(DataObjectTest, copy_constructor_test) {

    static_assert(
        std::is_copy_constructible<DataFloat>::value
    );

    DataFloat data_obj(1.0f, 1.0f);
    DataFloat data_obj2 = data_obj;

    ASSERT_FLOAT_EQ(data_obj2.getData_1(), 1.0f);
    ASSERT_FLOAT_EQ(data_obj2.getData_2(), 1.0f);

    ASSERT_EQ(data_obj2.COPY_CONSTRUCTOR_CALLED, 1);
    ASSERT_EQ(data_obj2.CONSTRUCTOR_CALLED, 0);

}

TEST(DataObjectTest, move_constructor_test) {
    static_assert(
        std::is_move_constructible<DataFloat>::value
    );

    DataFloat data_obj(1.0f, 1.0f);
    DataFloat data_obj2 = std::move(data_obj);

    ASSERT_EQ(data_obj2.CONSTRUCTOR_CALLED, 0);
    ASSERT_EQ(data_obj2.MOVE_CONSTRUCTOR_CALLED, 1);
}
